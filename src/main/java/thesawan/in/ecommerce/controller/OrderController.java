...
public class OrderController {
    ...
    public ResponseEntity<PaymentLinkResponse> createOrderHandler(
        ...
        if (paymentMethod.equals(PaymentMethod.RAZORPAY)) {
            ...
            String paymentLinkReferenceId = paymentLink.get("reference_id");
            String paymentLinkStatus = paymentLink.get("status");
            String razorpayPaymentId = paymentLink.get("payment_id");
            res.setPayment_link_url(paymentUrl);
            for (Order order : orders) {
                PaymentDetails details = new PaymentDetails();
                details.setPaymentId(null); // will be updated after payment success
                details.setRazorpayPaymentLinkId(paymentUrlId);
                details.setRazorpayPaymentLinkReferenceId(paymentLinkReferenceId);
                details.setRazorpayPaymentLinkStatus(paymentLinkStatus);
                details.setRazorpayPaymentId(razorpayPaymentId); // null initially
                details.setStatus(PaymentStatus.PENDING);
